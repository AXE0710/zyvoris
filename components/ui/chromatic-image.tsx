...
export function ChromaticImage({
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      previousTime = time;
      progress = approach(progress, progressTarget, 6, delta);
      pointer.x = approach(pointer.x, pointerTarget.x, 14, delta);
      pointer.y = approach(pointer.y, pointerTarget.y, 14, delta);
      ...
      const rotateX = reduceMotion ? 0 : (0.5 - pointer.y) * tilt * 12;
      const rotateY = reduceMotion ? 0 : (pointer.x - 0.5) * tilt * 12;
      canvas.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) scale(1.02)`;

      const isSettled =
        Math.abs(progress - progressTarget) < 0.0005 &&
  ...
  return (
    ...
    >
      <img
        ...
        alt={alt}
        className={cn(
          "absolute inset-0 size-full object-cover transition-opacity duration-300 pointer-events-none select-none",
      ...
      <canvas
        ...
        aria-hidden="true"
        className={cn(
          "absolute -inset-[3%] size-[106%] will-change-transform transition-opacity duration-300 pointer-events-none",
